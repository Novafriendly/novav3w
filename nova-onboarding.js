...
function begin(){
 ...
 q('[data-back]').onclick=()=>changeStep(-1);
 q('.nova-tour-next').onclick=async()=>{
  ...
  if(step===5)finish();else await changeStep(1)
 };
 async function changeStep(direction){
  if(busy)return;busy=true;setBusy(true);
  const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  try{
   if(!reduced)await body.animate([{opacity:1,transform:'translateX(0)'},{opacity:0,transform:'translateX('+(-direction*18)+'px)'}],{duration:130,easing:'ease-in',fill:'forwards'}).finished;
   body.getAnimations().forEach(a=>a.cancel());step+=direction;render();dialog.scrollTop=0;
   if(!reduced)await body.animate([{opacity:0,transform:'translateX('+(direction*22)+'px) scale(.985)'},{opacity:1,transform:'translateX(0) scale(1)'}],{duration:280,easing:'cubic-bezier(.2,.8,.2,1)'}).finished;
 ...
 function render(){
  ...
  if(step===2){body.insertAdjacentHTML('beforeend','<section class="nova-tour-profile"><div class="nova-tour-avatar"></div><strong data-profile-name></strong><small>Your Nova profile</small><div class="nova-tour-photo-actions"><button type="button" data-upload>＋ Upload photo</button><button type="button" data-remove>Use initial</button></div><input hidden type="file" accept="image/png,image/jpeg,image/webp,image/gif" aria-label="Upload profile photo"><small>PNG, JPG, WebP or GIF · Up to 2 MB</small></section><label class="nova-tour-link-label">Or use an image link<div class="nova-tour-link-row"><input type="url" placeholder="https://example.com/photo.jpg"><button type="button" data-photo>Preview</button></div></label>');q('[data-profile-name]').textContent=user;q('[data-upload]').onclick=()=>q('input[type=file]').click();q('[data-remove]').onclick=()=>{photo='';dirty=true;q('input[type=url]').value='';preview();status.textContent='Next saves your initial avatar.'};q('input[type=url]').value=photo.startsWith('http')?photo:'';preview();q('[data-photo]').onclick=()=>choosePhoto(q('input[type=url]').value.trim());q('input[type=file]').onchange=async e=>{const file=e.target.files[0];if(!file)return;if(!['image/png','image/jpeg','image/webp','image/gif'].includes(file.type)||file.size>2097152){status.textContent='Choose a PNG, JPG, WebP or GIF under 2 MB.';return}const reader=new FileReader();reader.onload=()=>choosePhoto(reader.result);reader.onerror=()=>status.textContent='Could not read that file.';reader.readAsDataURL(file)};}
